...
<p>With a pre-experiment covariate $X$ (last month's spend) correlated with the outcome $Y$, analyse $Y_{\\text{adj}} = Y - \\theta(X - \\mathbb{E}[X])$ with $\\theta = \\mathrm{Cov}(Y,X)/\\mathrm{Var}(X)$. The estimator stays unbiased and its variance falls by $(1-\\rho^2)$: at $\\rho = 0.6$ that is a <b>36% variance reduction</b>, so the same power arrives in 36% fewer samples — ten days instead of sixteen, for a line of arithmetic. It is the highest-leverage trick in experimentation and nearly always available, because pre-period behaviour usually predicts post-period behaviour.</p>
